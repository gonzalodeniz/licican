...
- `PB-012`: datos consolidados con licitaciones, lotes y adjudicaciones.
...
- `PB-005`: pipeline de seguimiento con alta, actualizacion de estado y deteccion de duplicados.
...
- `PB-013`: KPIs y matriz de permisos visibles para roles con acceso.
...
- `/datos-consolidados`: vista HTML de `PB-012` con licitaciones, lotes y adjudicaciones.
- `/api/datos-consolidados`: JSON de `PB-012`.
- `/datos-consolidados/licitaciones/<id>`: detalle HTML de una licitacion consolidada.
- `/datos-consolidados/adjudicaciones/<id>`: detalle HTML de una adjudicacion consolidada.
...
- `/pipeline`: vista HTML del seguimiento operativo de oportunidades.
- `/api/pipeline`: JSON del seguimiento operativo.
...
- `/kpis`: vista HTML de indicadores de cobertura, adopcion y uso.
- `/permisos`: vista HTML de la matriz funcional de roles y permisos.
...
- `/api/datos-consolidados`: devuelve `referencia_funcional`, `archivo_origen`, `resumen`, `licitaciones`, `lotes` y `adjudicaciones`.
- `/api/pipeline`: devuelve `referencia_funcional`, `summary`, `estados_disponibles` y `pipeline`.
- `/kpis`: devuelve una tarjeta resumen con cobertura visible, adopcion de alertas y uso de pipeline.
- `/permisos`: devuelve una matriz resumida de consultas, gestion y gobierno por rol.
...
- [canarias_dataset.py](/opt/apps/licican/src/licican/canarias_dataset.py) lee el Excel versionado de `PB-012` y expone licitaciones, lotes y adjudicaciones.
...
- [pipeline.py](/opt/apps/licican/src/licican/pipeline.py) persiste el seguimiento operativo por usuario y oportunidad, evita duplicados y valida estados.
...
- `pyproject.toml` sigue describiendo el paquete como una release centrada solo en cobertura de fuentes, aunque la rama ya expone catalogo inicial, filtros, detalle, datos consolidados, pipeline, alertas, clasificacion TI auditable, KPIs, permisos y consolidacion Atom con respaldo JSON.
- La documentacion funcional de `product-manager/` describe backlog posterior valido, pero no debe leerse como contrato tecnico ya implementado para futuras ampliaciones de cobertura.
...
- `PB-012` y `PB-005` ya estan integrados en `main`; si un documento los presenta como ausentes, ese texto debe considerarse desactualizado.
...
- El pipeline y los datos consolidados ya estan expuestos en la superficie tecnica revisada.
...
- Mantener sincronizada la metadata de paquete si se quiere que describa fielmente la superficie observable de `main`.
- Definir la evolucion funcional de autenticacion y de notificaciones si se quiere pasar de control por rol simulado a operacion real.
