...
| PB-013 | Modelo funcional de roles y permisos | La aplicacion debe aplicar permisos coherentes para `Administrador`, `Colaborador` y `Lector/Invitado` sobre acciones de consulta, alertas, pipeline y gobierno funcional, evitando exponer controles operativos que el rol no puede usar. | P1 | Reduce ambiguedad operativa, prepara crecimiento multiusuario y evita fugas funcionales entre consulta y gestion. | 1. El sistema distingue al menos los roles `Administrador`, `Colaborador` y `Lector/Invitado`. 2. `Administrador` puede consultar, gestionar alertas, gestionar pipeline, consultar KPIs y administrar roles o permisos. 3. `Colaborador` puede consultar, gestionar sus alertas y su pipeline propio, y consultar KPIs solo si no exponen informacion sensible. 4. `Lector/Invitado` solo puede consultar catalogo, detalle, filtros y vistas consolidadas. 5. Las acciones no permitidas quedan bloqueadas de forma consistente y no se muestran como disponibles. 6. Existe una issue ejecutable para `developer-teams` con escenarios verificables por rol. | PB-010, PB-004, PB-005 | `nuevo` | HU-13, CU-13, issue #13 |
...
- `PB-013` entra como `P1` porque la matriz de permisos ya esta definida funcionalmente y debe traducirse en comportamiento observable antes de ampliar gestion multiusuario o administracion.
